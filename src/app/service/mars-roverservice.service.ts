...
export class MarsRoverserviceService {
  ...
  findByDateCamera(
    roboRovesr: Observable<MarsPhoto[]>,
    data: Observable<MarsPhoto[]>,
    marsCameras: Observable<MarsPhoto[]>,
    solMars: Observable<MarsPhoto[]>
  ) {
    ...
    let request = this.http.get<MarsPhoto[]>(urlCenter)
    console.log(request)
